...
class _UserOverviewScreenState extends State<UserOverviewScreen> {
  ...
  Future<void> _openVersePicker(
    ...
    final selectedAyahIds = await showModalBottomSheet<List<int>>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _SurahVersePickerSheet(
        surahName: surahCard.surahName,
...
// ─────────────────────────────────────────────────────────────────────────────
// Surah Verse Picker Sheet — multi-select checkboxes
// ─────────────────────────────────────────────────────────────────────────────
class _SurahVersePickerSheet extends StatefulWidget {
  const _SurahVersePickerSheet({
  ...
  @override
  State<_SurahVersePickerSheet> createState() =>
      _SurahVersePickerSheetState();
}

class _SurahVersePickerSheetState extends State<_SurahVersePickerSheet> {
...
class _VersePickerRow extends StatelessWidget {
  const _VersePickerRow({
    required this.entry,
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 10),
            // Verse text (one line, truncated) + verse number
            Expanded(
              child: Text(
                entry.text,
                textDirection: TextDirection.rtl,
