...
> Keystore secrets must stay outside the repository. The current keystore uses alias `upload`; retrieve the password from the secure local secret store before signing.
...
```powershell
keytool -export -rfc -keystore C:\1stD\Public-data\UP.Key.jks -storepass "<keystore-password>" -alias upload -file android/upload_certificate_reset_2026.pem
```

Check the new key fingerprint:

```powershell
keytool -list -v -keystore C:\1stD\Public-data\UP.Key.jks -storepass "<keystore-password>" -alias upload
```
